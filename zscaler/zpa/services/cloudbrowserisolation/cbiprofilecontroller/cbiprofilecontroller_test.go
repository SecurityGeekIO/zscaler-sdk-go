...
import (
	...
	"github.com/SecurityGeekIO/zscaler-sdk-go/v3/zscaler/zpa/services/cloudbrowserisolation/cbiregions"
...
func TestCBIProfileController(t *testing.T) {
	...
	cbiCertificate, _, err := cbicertificatecontroller.GetByName(context.Background(), service, "Zscaler Root Certificate")
	if err != nil {
	...
	cbiBannerController, _, err := cbibannercontroller.Create(context.Background(), service, &cbiBanner)
	...
	defer func() {
		if cbiBannerController != nil && cbiBannerController.ID != "" {
			existingCbiBanner, _, errCheck := cbibannercontroller.Get(context.Background(), service, cbiBannerController.ID)
			if errCheck == nil && existingCbiBanner != nil {
				_, errDelete := cbibannercontroller.Delete(context.Background(), service, cbiBannerController.ID)
				if errDelete != nil {
	...
	createdResource, _, err := Create(context.Background(), service, &cbiProfile)
	...
	createdResource, _, err = Get(context.Background(), service, createdResource.ID)
	if err != nil {
