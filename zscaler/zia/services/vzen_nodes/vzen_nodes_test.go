...
func TestVZENNodes(t *testing.T) {
	...
	vzenNode := VZENNodes{
		...
		LoadBalancerIPAddress: "10.0.0.10",
		DeploymentMode:        "CLUSTER",
		VzenSkuType:           "LARGE",
	}

	var createdResource *VZENNodes

	// Test resource creation
	err = retryOnConflict(func() error {
		createdResource, _, err = Create(context.Background(), service, &vzenNode)
		return err
	})
	if err != nil {
	...
	if createdResource.Name != name {
		t.Errorf("Expected created vzen node '%s', but got '%s'", name, createdResource.Name)
	}
	...
	if retrievedResource.Name != name {
		t.Errorf("Expected retrieved vzen node '%s', but got '%s'", name, retrievedResource.Name)
	}
	...
	retrievedResource, err = GetNodeByName(context.Background(), service, name)
	if err != nil {
...
func tryRetrieveResource(ctx context.Context, service *zscaler.Service, id int) (*VZENNodes, error) {
	var resource *VZENNodes
	var err error
...
func TestUpdateNonExistentResource(t *testing.T) {
	...
	_, _, err = Update(context.Background(), service, 0, &VZENNodes{})
	if err == nil {
...
func TestGetByNameNonExistentResource(t *testing.T) {
	...
	_, err = GetNodeByName(context.Background(), service, "non_existent_name")
	if err == nil {
