...
func TestBandwidthClasses(t *testing.T) {
	...
	rule := BandwidthClasses{
		Name:            name,
		WebApplications: []string{"ACADEMICGPT", "AD_CREATIVES"},
		Urls:            []string{"zscaler.com", "zscaler.net"},
		UrlCategories:   []string{"ADULT_THEMES", "ADULT_SEX_EDUCATION"},
	}

	var createdResource *BandwidthClasses

	// Test resource creation
	err = retryOnConflict(func() error {
		createdResource, _, err = Create(context.Background(), service, &rule)
		return err
	})
	if err != nil {
	...
	if retrievedResource.Name != name {
		t.Errorf("Expected retrieved bandwidth control classes '%s', but got '%s'", name, retrievedResource.Name)
	}
...
func tryRetrieveResource(s *zscaler.Service, id int) (*BandwidthClasses, error) {
	var resource *BandwidthClasses
	var err error
...
func TestUpdateNonExistentResource(t *testing.T) {
	...
	_, _, err = Update(context.Background(), service, 0, &BandwidthClasses{})
	if err == nil {
