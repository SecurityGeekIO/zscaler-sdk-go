...
const (
	adminUsersEndpoint = "/ztw/api/v1/adminUsers"
)
...
func CreateAdminUser(ctx context.Context, service *zscaler.Service, adminUser AdminUsers) (*AdminUsers, error) {
	resp, err := service.Client.CreateResource(ctx, adminUsersEndpoint, adminUser)
	if err != nil {
		return nil, err
	}
...
func UpdateAdminUser(ctx context.Context, service *zscaler.Service, adminUserID int, adminUser AdminUsers) (*AdminUsers, error) {
	...
	resp, err := service.Client.UpdateWithPutResource(ctx, path, adminUser)
	if err != nil {
		return nil, err
	}
...
func DeleteAdminUser(ctx context.Context, service *zscaler.Service, adminUserID int) (*http.Response, error) {
	err := service.Client.DeleteResource(ctx, fmt.Sprintf("%s/%d", adminUsersEndpoint, adminUserID))
	if err != nil {
		return nil, err
	}
