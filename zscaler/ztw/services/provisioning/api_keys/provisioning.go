...
const (
	apiKeysEndpoint           = "/ztw/api/v1/apiKeys"
	regenerateApiKeysEndpoint = "/ztw/api/v1/regenerate"
)
...
func Get(ctx context.Context, service *zscaler.Service, apiKeyID int) (*ProvisioningAPIKeys, error) {
	var apiKey ProvisioningAPIKeys
	err := service.Client.ReadResource(ctx, fmt.Sprintf("%s/%d", apiKeysEndpoint, apiKeyID), &apiKey)
	if err != nil {
		return nil, err
	}
...
func GetPartnerAPIKey(ctx context.Context, service *zscaler.Service, apiKeyValue string, includePartnerKey bool) (*ProvisioningAPIKeys, error) {
	...
	err := service.Client.ReadResource(ctx, url, &apiKeys)
	if err != nil {
		return nil, err
	}
...
func GetAll(ctx context.Context, service *zscaler.Service) ([]ProvisioningAPIKeys, error) {
	var apiKeys []ProvisioningAPIKeys
	err := common.ReadAllPages(ctx, service.Client, apiKeysEndpoint, &apiKeys)
	return apiKeys, err
}

func Create(ctx context.Context, service *zscaler.Service, apiKeyValue *ProvisioningAPIKeys, includePartnerKey bool, keyId *int) (*ProvisioningAPIKeys, error) {
	...
	resp, err := service.Client.CreateResource(ctx, url, *apiKeyValue)
	if err != nil {
		return nil, err
	}
