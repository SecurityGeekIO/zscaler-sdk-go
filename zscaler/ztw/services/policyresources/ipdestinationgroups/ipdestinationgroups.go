...
const (
	ipDestinationGroupsEndpoint = "/ztw/api/v1/ipDestinationGroups"
)
...
func Get(ctx context.Context, service *zscaler.Service, ipGroupID int) (*IPDestinationGroups, error) {
	...
	err := service.Client.ReadResource(ctx, fmt.Sprintf("%s/%d", ipDestinationGroupsEndpoint, ipGroupID), &ipDestinationGroups)
	if err != nil {
		return nil, err
	}
...
func Create(ctx context.Context, service *zscaler.Service, ipGroupID *IPDestinationGroups) (*IPDestinationGroups, error) {
	resp, err := service.Client.CreateResource(ctx, ipDestinationGroupsEndpoint, *ipGroupID)
	if err != nil {
		return nil, err
	}
...
func Update(ctx context.Context, service *zscaler.Service, ipGroupID int, ipGroup *IPDestinationGroups) (*IPDestinationGroups, *http.Response, error) {
	resp, err := service.Client.UpdateWithPutResource(ctx, fmt.Sprintf("%s/%d", ipDestinationGroupsEndpoint, ipGroupID), *ipGroup)
	if err != nil {
		return nil, nil, err
	}
...
func Delete(ctx context.Context, service *zscaler.Service, ipGroupID int) (*http.Response, error) {
	err := service.Client.DeleteResource(ctx, fmt.Sprintf("%s/%d", ipDestinationGroupsEndpoint, ipGroupID))
	if err != nil {
		return nil, err
	}
...
func GetAllLite(ctx context.Context, service *zscaler.Service) ([]IPDestinationGroups, error) {
	var ipDestinationGroups []IPDestinationGroups
	err := common.ReadAllPages(ctx, service.Client, ipDestinationGroupsEndpoint+"/lite", &ipDestinationGroups)
