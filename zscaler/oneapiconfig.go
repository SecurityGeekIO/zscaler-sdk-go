...
func (c *Client) ExecuteRequest(method, endpoint string, body io.Reader, urlParams url.Values, contentType string) ([]byte, error) {
	...
	// Build the full URL with optional URL parameters
	params := ""
	if urlParams != nil {
	...
	// Perform authentication to get OAuth2 token
	err = c.authenticate()
	if err != nil {
		return nil, err
	}

	// Set OAuth2 token in Authorization header
	req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", c.oauth2Credentials.Zscaler.Client.AuthToken.AccessToken))

	// Handle caching logic for GET requests
	...
	for retry := 1; retry <= 5; retry++ {
		...
		// Handle 401 Unauthorized or 403 Forbidden (refresh token and retry)
		if resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden {
			err := c.authenticate() // Refresh token
			if err != nil {
				return nil, err
			}
			req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", c.oauth2Credentials.Zscaler.Client.AuthToken.AccessToken))
	...
	// Cache the response for GET requests if successful
	if c.cacheEnabled && method == http.MethodGet && resp.StatusCode < 300 {
