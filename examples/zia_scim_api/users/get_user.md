...
func main() {
	...
	fetchedUser, _, err := scim_api.GetUser(ctx, service, "83b86518-77f3-4059-9aa5-be999c5d1fc9")
	if err != nil {
		log.Fatalf("Error fetching SCIM user: %v", err)
	}
	log.Printf("User fetched: ID=%s, Department=%s", fetchedUser.ID, fetchedUser.EnterpriseExtension.Department)
}
```