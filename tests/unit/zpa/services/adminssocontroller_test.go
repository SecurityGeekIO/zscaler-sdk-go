...
import (
	...
	"github.com/SecurityGeekIO/zscaler-sdk-go/v3/zscaler/zpa/services/admin_sso_controller"
...
func TestAdminSSOController_UpdateSSOLoginController_SDK(t *testing.T) {
	server := common.NewTestServer()
	defer server.Close()

	path := "/zpa/mgmtconfig/v1/admin/customers/" + testCustomerID + "/v2/ssoLoginOptions"

	server.On("POST", path, common.SuccessResponse(admin_sso_controller.AdminSSOLoginOptions{
		SSOLoginOnly: true,
	}))
	...
	updateSettings := &admin_sso_controller.AdminSSOLoginOptions{
		SSOLoginOnly: true,
	}

	result, _, err := admin_sso_controller.UpdateSSOLoginController(context.Background(), service, updateSettings)
