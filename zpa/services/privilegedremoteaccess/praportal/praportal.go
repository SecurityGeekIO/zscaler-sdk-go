...
func Create(service *services.Service, sraPortal *PRAPortal) (*PRAPortal, *http.Response, error) {
	v := new(PRAPortal)
	resp, err := service.NewRequestDo("POST", mgmtConfig+service.Client.GetCustomerID()+praPortalEndpoint, common.Filter{MicroTenantID: service.MicroTenantID()}, sraPortal, &v)
	if err != nil {
		return nil, nil, err
	}
	return v, resp, nil
...
func Update(service *services.Service, portalID string, sraPortal *PRAPortal) (*http.Response, error) {
	...
	resp, err := service.NewRequestDo("PUT", path, common.Filter{MicroTenantID: service.MicroTenantID()}, sraPortal, nil)
	if err != nil {
		return nil, err
	}
	return resp, err
