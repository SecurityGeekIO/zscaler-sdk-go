...
func Create(service *services.Service, browserAccess BrowserAccess) (*BrowserAccess, *http.Response, error) {
	v := new(BrowserAccess)
	resp, err := service.NewRequestDo("POST", mgmtConfig+service.Client.GetCustomerID()+browserAccessEndpoint, common.Filter{MicroTenantID: service.MicroTenantID()}, browserAccess, &v)
	if err != nil {
		return nil, nil, err
	}
	return v, resp, nil
...
func Update(service *services.Service, appID string, browserAccess *BrowserAccess) (*http.Response, error) {
	...
	resp, err := service.NewRequestDo("PUT", path, common.Filter{MicroTenantID: service.MicroTenantID()}, browserAccess, nil)
	if err != nil {
		return nil, err
	}
	return resp, err
