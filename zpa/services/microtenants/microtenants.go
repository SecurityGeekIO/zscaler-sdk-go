...
func Create(service *services.Service, microTenant MicroTenant) (*MicroTenant, *http.Response, error) {
	v := new(MicroTenant)
	resp, err := service.NewRequestDo("POST", mgmtConfig+service.Client.GetCustomerID()+microtenantsEndpoint, nil, microTenant, &v)
	if err != nil {
		return nil, nil, err
	}
...
func Update(service *services.Service, microTenantID string, microTenant *MicroTenant) (*http.Response, error) {
	...
	resp, err := service.NewRequestDo("PUT", relativeURL, nil, microTenant, nil)
	if err != nil {
		return nil, err
	}
