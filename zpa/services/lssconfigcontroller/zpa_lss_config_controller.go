...
func Create(service *services.Service, lssConfig *LSSResource) (*LSSResource, *http.Response, error) {
	v := new(LSSResource)
	resp, err := service.NewRequestDo("POST", mgmtConfig+service.Client.GetCustomerID()+lssConfigEndpoint, nil, lssConfig, &v)
	if err != nil {
		return nil, nil, err
	}
	return v, resp, nil
...
func Update(service *services.Service, lssID string, lssConfig *LSSResource) (*http.Response, error) {
	...
	resp, err := service.NewRequestDo("PUT", path, nil, lssConfig, nil)
	if err != nil {
		return nil, err
	}
	return resp, err
