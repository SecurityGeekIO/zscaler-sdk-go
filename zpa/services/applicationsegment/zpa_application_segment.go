...
func Create(service *services.Service, appSegment ApplicationSegmentResource) (*ApplicationSegmentResource, *http.Response, error) {
	v := new(ApplicationSegmentResource)
	resp, err := service.NewRequestDo("POST", mgmtConfig+service.Client.GetCustomerID()+appSegmentEndpoint, common.Filter{MicroTenantID: service.MicroTenantID()}, appSegment, &v)
	if err != nil {
		return nil, nil, err
	}
...
func Update(service *services.Service, appID string, appSegmentRequest ApplicationSegmentResource) (*http.Response, error) {
	...
	resp, err := service.NewRequestDo("PUT", relativeURL, common.Filter{MicroTenantID: service.MicroTenantID()}, appSegmentRequest, nil)
	if err != nil {
		return nil, err
	}
...
func Delete(service *services.Service, appID string) (*http.Response, error) {
	relativeURL := fmt.Sprintf("%s/%s", mgmtConfig+service.Client.GetCustomerID()+appSegmentEndpoint, appID)
	resp, err := service.NewRequestDo("DELETE", relativeURL, common.DeleteApplicationQueryParams{ForceDelete: true, MicroTenantID: service.MicroTenantID()}, nil, nil)
	if err != nil {
		return nil, err
	}
