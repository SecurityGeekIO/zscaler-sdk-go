...
func TestNonExistentResourceOperations(t *testing.T) {
	...
	service := New(client) // Ensure this New() function is correctly pointing to your service constructor

	for _, policyType := range policyTypes {
		t.Run(policyType, func(t *testing.T) {
			// Testing retrieve non-existent resource
			_, _, err = service.GetPolicyRule(policyType, "non_existent_id")
			if err == nil {
			...
			_, err = service.Delete(policyType, "non_existent_id")
			if err == nil {
			...
			_, err = service.UpdateRule(policyType, "non_existent_id", &PolicyRule{})
			if err == nil {
			...
			_, _, err = service.GetByNameAndType(policyType, "non_existent_name")
			if err == nil {
