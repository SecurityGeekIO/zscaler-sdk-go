...
func init() {
	...
	for _, r := range resources {
		if !strings.HasPrefix(r.Name, "tests-rule3") {
			continue
		}
...
func TestPolicyAccessRule3(t *testing.T) {
	name := "tests-rule3" + acctest.RandStringFromCharSet(10, acctest.CharSetAlpha)
	updateName := "tests-rule3" + acctest.RandStringFromCharSet(10, acctest.CharSetAlpha)
	client, err := tests.NewZpaClient()
	if err != nil {
