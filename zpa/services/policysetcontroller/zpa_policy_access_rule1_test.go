...
const accessPolicyType = "ACCESS_POLICY"
...
func init() {
	...
	for _, r := range resources {
		if !strings.HasPrefix(r.Name, "tests-rule1") {
			continue
		}
...
func TestPolicyAccessRule(t *testing.T) {
	...
	name := "tests-rule1" + acctest.RandStringFromCharSet(10, acctest.CharSetAlpha)
	updateName := "tests-rule1" + acctest.RandStringFromCharSet(10, acctest.CharSetAlpha)
	client, err := tests.NewZpaClient()
	if err != nil {
