...
func GetPacFiles(service *services.Service, filter string) ([]PACFileConfig, error) {
	...
	err := common.ReadAllPages(service.Client, endpoint, &pacFiles)
	return pacFiles, err
}

func GetPacFileByName(service *services.Service, pacFileName string) (*PACFileConfig, error) {
	var pacFiles []PACFileConfig
	err := common.ReadAllPages(service.Client, pacfileEndpoint, &pacFiles)
	if err != nil {
		return nil, err
	}
...
func GetPacFileVersion(service *services.Service, pacID int, filter string) ([]PACFileConfig, error) {
	...
	err := service.Client.Read(endpoint, &pacFiles)
	if err != nil {
		return nil, err
	}
	...
	for _, pacFile := range pacFiles {
		service.Client.Logger.Printf("[DEBUG] Returning PAC file version ID: %d", pacFile.ID)
	}

	return pacFiles, nil
}

func GetPacVersionID(service *services.Service, pacID, pacVersion int, filter string) (*PACFileConfig, error) {
	...
	err := service.Client.Read(endpoint, &pacFile)
	if err != nil {
...
func UpdatePacFile(service *services.Service, pacID, pacVersion int, pacVersionAction string, file *PACFileConfig, newLKGVer *int) (*PACFileConfig, error) {
	...
	resp, err := service.Client.UpdateWithPut(endpoint, *file)
	if err != nil {
	...
	service.Client.Logger.Printf("[DEBUG] Returning updated PAC file: %d", updatedPacFile.ID)

	return updatedPacFile, nil
}

func DeletePacFile(service *services.Service, fileID int) (*http.Response, error) {
	err := service.Client.Delete(fmt.Sprintf("%s/%d", pacfileEndpoint, fileID))
	if err != nil {
...
func ValidatePacFile(service *services.Service, pacContent string) (*PacResult, error) {
	...
	resp, err := service.Client.CreateWithRawPayload(url, pacContent)
	if err != nil {
	...
	// Log the result for debugging
	service.Client.Logger.Printf("[DEBUG] PAC validation result: %+v", validationResult)
...
func CreateClonedPacFileVersion(service *services.Service, pacID int, clonedPacVersion int, deleteVersion *int, file *PACFileConfig) (*PACFileConfig, error) {
	...
	// Log the result for debugging
	service.Client.Logger.Printf("[DEBUG] Returning PAC file version from create: %d", createdPacFile.ID)
